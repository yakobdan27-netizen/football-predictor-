/**
 * Fill Serie A 2026/27 team season cards from DB/seed/live only.
...
export function countLiveSaMatches(
  ...
  for (const batch of batches) {
    for (const match of batch.matches) {
      ...
      if (league !== SA_LEAGUE_NAME) continue;
      const date = match.matchDate ?? batch.date;
      if (season === SA_SEASON_2026_27) {
...
export function fillSaTeamSeasonCard(
  team: string,
  opts?: {
  ...
  const season = opts?.season ?? SA_SEASON_2026_27;
  const name = standardizeTeamName(team);
  const is_promoted = isSaPromotedTeam(name);
  const base = emptySaTeamSeasonCard(name, { seed_paused: opts?.seed_paused });
  ...
  const live = primaryFtSeasonStats(
    ...
    countLiveSaMatches(name, opts?.batches ?? [], season)
  );
...
function liveVenueSplits(
  ...
  for (const batch of batches) {
    for (const match of batch.matches) {
      if (matchLeague(match, batch.league) !== SA_LEAGUE_NAME) continue;
      const date = match.matchDate ?? batch.date;
      if (season === SA_SEASON_2026_27) {
        if (date < "2026-08-01" || date >= "2027-08-01") continue;
      }
...
export function buildAllSaSeasonCards(
  batches?: PredictionBatch[],
  pausedTeams?: Set<string>,
  teamList?: string[],
  systemSeasonByTeam?: Map<string, SystemSeasonTeamMatchStats>
): Record<string, SaTeamSeasonCard> {
  const list = teamList?.length ? teamList : SA_2026_27_PROVISIONAL_TEAMS;
  const cards: Record<string, SaTeamSeasonCard> = {};
  for (const team of list) {
    cards[team] = fillSaTeamSeasonCard(team, {
      batches,
...
export async function buildAllSaSeasonCardsAsync(
  batches?: PredictionBatch[],
  pausedTeams?: Set<string>,
  teamList?: string[]
): Promise<Record<string, SaTeamSeasonCard>> {
  const list = teamList?.length ? teamList : SA_2026_27_PROVISIONAL_TEAMS;
