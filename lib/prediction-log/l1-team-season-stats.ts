...
export function fillL1TeamSeasonCard(
  ...
  const live = primaryFtSeasonStats(
    ...
    countLiveL1Matches(name, opts?.batches ?? [], season)
  );
...
export async function buildAllL1SeasonCardsAsync(
  batches?: PredictionBatch[],
  pausedTeams?: Set<string>,
  teamList?: string[]
): Promise<Record<string, L1TeamSeasonCard>> {
  const list = teamList ?? [];
