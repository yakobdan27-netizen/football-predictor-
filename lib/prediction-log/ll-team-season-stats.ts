/**
 * Fill La Liga 2026/27 team season cards from DB/seed/live only.
...
export function countLiveLlMatches(
  ...
  for (const batch of batches) {
    for (const match of batch.matches) {
      ...
      if (league !== LL_LEAGUE_NAME) continue;
      const date = match.matchDate ?? batch.date;
      if (season === LL_SEASON_2026_27) {
...
export function fillLlTeamSeasonCard(
  team: string,
  opts?: {
  ...
  const base = emptyLlTeamSeasonCard(name, { seed_paused: opts?.seed_paused });
...
function liveVenueSplits(
  ...
  for (const batch of batches) {
    for (const match of batch.matches) {
      if (matchLeague(match, batch.league) !== LL_LEAGUE_NAME) continue;
      const date = match.matchDate ?? batch.date;
      if (season === LL_SEASON_2026_27) {
        if (date < "2026-08-01" || date >= "2027-08-01") continue;
      }
