/**
 * Fill Bundesliga 2026/27 team season cards from DB/seed/live only.
...
export function countLiveBlMatches(
  ...
  for (const batch of batches) {
    for (const match of batch.matches) {
      ...
      if (league !== BL_LEAGUE_NAME) continue;
      const date = match.matchDate ?? batch.date;
      if (season === BL_SEASON_2026_27) {
...
export function fillBlTeamSeasonCard(
  team: string,
  opts?: {
  ...
  const is_promoted = isBlPromotedTeam(name, opts?.rosterTeams);
  const base = emptyBlTeamSeasonCard(name, {
...
function liveVenueSplits(
  ...
  for (const batch of batches) {
    for (const match of batch.matches) {
      if (matchLeague(match, batch.league) !== BL_LEAGUE_NAME) continue;
      const date = match.matchDate ?? batch.date;
      if (season === BL_SEASON_2026_27) {
        if (date < "2026-08-01" || date >= "2027-08-01") continue;
      }
...
export function buildAllBlSeasonCards(
  ...
  for (const team of list) {
    cards[team] = fillBlTeamSeasonCard(team, {
      batches,
