...
import {
  hydrateLlSeasonRosterFromServer,
  saveLlSeasonRoster,
} from "@/lib/prediction-log/storage";

function filledCount(card: LlTeamSeasonCard): number {
  const fields = [
    card.matches_played,
...
export function LlSeasonRosterCard() {
  const [store, setStore] = useState<LlSeasonRosterStore | null>(null);
  const [busy, setBusy] = useState(false);
  const [msg, setMsg] = useState<string | null>(null);

  const refresh = useCallback(async () => {
    const s = await hydrateLlSeasonRosterFromServer();
    setStore(s);
  }, []);
  ...
  if (!store) {
    return <p className="page-sub">Loading La Liga 2026/27 roster…</p>;
  }

  const cards = store.teams
    .map((t) => store.cards[t])
    .filter(Boolean) as LlTeamSeasonCard[];
