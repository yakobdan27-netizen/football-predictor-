...
import type { PlSeasonRosterStore, PlTeamSeasonCard } from "@/lib/prediction-log/pl-season-roster";
import {
  hydratePlSeasonRosterFromServer,
  savePlSeasonRoster,
} from "@/lib/prediction-log/storage";

function filledCount(card: PlTeamSeasonCard): number {
  const fields = [
    card.matches_played,
...
export function PlSeasonRosterCard() {
  const [store, setStore] = useState<PlSeasonRosterStore | null>(null);
  const [busy, setBusy] = useState(false);
  const [msg, setMsg] = useState<string | null>(null);

  const refresh = useCallback(async () => {
    const s = await hydratePlSeasonRosterFromServer();
    setStore(s);
  }, []);
  ...
  if (!store) {
    return <p className="page-sub">Loading PL 2026/27 roster…</p>;
  }

  const cards = store.teams.map((t) => store.cards[t]).filter(Boolean) as PlTeamSeasonCard[];
      ...
      >
        <div>
          <h2 style={{ fontSize: "1.05rem", fontWeight: 700, margin: 0 }}>
            Premier League 2026/27 roster
          </h2>
          <p style={{ fontSize: "0.8125rem", color: "var(--muted)", margin: "0.35rem 0 0" }}>
            Numerics from DB/live only (null if missing). Style seeds are qualitative. Never blocks
            markets.
          </p>
        </div>
