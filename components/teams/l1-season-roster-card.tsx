...
import {
  hydrateL1SeasonRosterFromServer,
  saveL1SeasonRoster,
} from "@/lib/prediction-log/storage";

function filledCount(card: L1TeamSeasonCard): number {
  const fields = [
    card.matches_played,
...
export function L1SeasonRosterCard() {
  const [store, setStore] = useState<L1SeasonRosterStore | null>(null);
  const [busy, setBusy] = useState(false);
  const [msg, setMsg] = useState<string | null>(null);

  const refresh = useCallback(async () => {
    const s = await hydrateL1SeasonRosterFromServer();
    setStore(s);
  }, []);
  ...
  if (!store) {
    return <p className="page-sub">Loading Ligue 1 2026/27 roster…</p>;
  }

  const cards = store.teams
    .map((t) => store.cards[t])
    .filter(Boolean) as L1TeamSeasonCard[];
